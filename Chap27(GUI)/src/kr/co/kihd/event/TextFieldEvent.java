...
public class TextFieldEvent extends Frame implements WindowListener, ActionListener {
	...
	public TextFieldEvent(String title) {
		super(title); 		// 조상 클래스인 Frame(String title)을 호출함.
		// 리스너 등록
	...
	public void textFieldShow() {
		...
		this.tfPwd.setEchoChar('*');     // 비밀번호 안 보이게 설정
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(!id.equals("gumi")) {
			System.out.println("입력하신 ID가 유효 하지 않습니다.");
			tfId.requestFocus(); 		// 마우스 커서를 아이디창으로 보냄.
			tfId.selectAll(); 			// 입력한 글자를 전부 선택 되게 함.
